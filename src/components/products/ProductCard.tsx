...
export default function ProductCard({ product }: { product: ProductItem }) {
  return (
    <div className="group bg-white border border-steel/10 hover:border-industrial-red/40 transition-all duration-500 hover:shadow-2xl overflow-hidden flex flex-col h-full transform hover:-translate-y-1">
      <div className="relative aspect-[4/3] w-full bg-near-black overflow-hidden p-2 flex items-center justify-center">
        <Image
          src={product.image}
          alt={product.name}
          fill
          className="object-contain p-2 group-hover:scale-110 transition-transform duration-700 ease-out animate-image-fade"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
      </div>
      <div className="p-6 md:p-8 flex-grow flex flex-col justify-between bg-white relative z-10">
        <div>
          <h4 className="font-heading font-bold text-xl md:text-2xl text-charcoal uppercase tracking-widest mb-3 group-hover:text-industrial-red transition-colors duration-300">
            {product.name}
          </h4>
